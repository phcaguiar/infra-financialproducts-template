...
# resource "azurerm_lb_rule" "lb_rule-gps-2" {
#   name                           = "${var.gps_prefix_name}-${var.env_prefix}-2"
#   resource_group_name            = "${var.product_rg_name}"
#   loadbalancer_id                = "${azurerm_lb.lb-gps.id}"
#   protocol                       = "${var.lb_rule_protocol}"
#   frontend_port                  = "${var.lb_rule_frontend_port2}"
#   backend_port                   = "${var.lb_rule_backend_port2}"
#   frontend_ip_configuration_name = "${var.gps_prefix_name}-${var.env_prefix}"
#   enable_floating_ip             = false
#   backend_address_pool_id        = "${azurerm_lb_backend_address_pool.backend_pool-gps.id}"
#   idle_timeout_in_minutes        = "${var.lb_rule_idle_timeout_in_minutes}"
# }
...
resource "azurerm_network_interface" "windows_vm_network_interface" {
  ...
  ip_configuration {
    name                          = "${var.windows_vm_name}0${count.index + 1}"
    subnet_id                     = "${data.azurerm_subnet.internal_subnet.id}"
    private_ip_address_allocation = "${var.windows_vm_network_interface_private_ip_address_allocation}"#dynamic
    load_balancer_backend_address_pools_ids = ["${azurerm_lb_backend_address_pool.lb_backend_address_pool.id}"]
  }
}
